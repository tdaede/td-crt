...
impl Serial {
    pub fn new(usart: USART1, gpioc: &GPIOC, rcc: &RCC) -> Serial {
        ...
        usart.brr().write(|w| { unsafe { w.brr().bits((APB2_CLOCK*2*8/16/1_000_000) as u16)} });
        usart.cr1().write(|w| { w.te().set_bit().re().set_bit().ue().set_bit().rxneie().set_bit().fifoen().set_bit() });
        Serial {
